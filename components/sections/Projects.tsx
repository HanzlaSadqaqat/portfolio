  ...
  return (
    <section id="projects" className="px-6 py-20 md:py-28 bg-bg-soft">
      <Container>
        ...
        <StaggerGrid className="grid md:grid-cols-2 gap-5 mt-12" stagger={0.1}>
          {projects.map((project) => (
            <StaggerItem key={project.name}>
              <motion.div
                whileHover={{ y: -4, transition: { duration: 0.2, ease } }}
                className="bg-bg-card border border-bg-border rounded-2xl overflow-hidden group h-full flex flex-col"
              >
